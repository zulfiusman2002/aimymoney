  ...
  return (
    <div className="page">
      <div className="fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 14, marginBottom: 24 }}>
        <div>
          <div className="t-label">What the money is actually for</div>
          <h1 style={{ fontSize: '2.3rem', marginTop: 6 }}>Goals</h1>
        </div>
        <button className="btn btn-gold" onClick={() => setForm({ goal_type: 'Custom' })}>+ New goal</button>
      </div>
            ...
            return (
              <div key={g.id} className="card" style={{ opacity: achieved ? 0.75 : 1 }}>
                ...
                {t.onTrack === false && t.requiredMonthly != null && (
                  <p style={{ fontSize: '.74rem', color: 'var(--c-red)', marginTop: 8 }}>
                ...
                {!achieved && t.remaining > 0 && (
                  <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--c-border)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.68rem', color: 'var(--c-muted)' }}>
                      ...
                      {saved > 0 && <span style={{ color: 'var(--c-green)' }}>−{saved} months ({scenMonths} mo total)</span>}
                    </div>
