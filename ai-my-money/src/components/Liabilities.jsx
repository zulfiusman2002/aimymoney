  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
        <div>
          <div className="t-label">Liabilities</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.3rem', fontWeight: 600, color: 'var(--c-red)' }}>−{f(total)}</div>
          {monthly > 0 && <div style={{ fontSize: '.68rem', color: 'var(--c-muted)' }}>{f(monthly)}/mo in payments</div>}
        </div>
        <button className="btn btn-xs btn-secondary" onClick={() => setForm({ type: 'loan' })}>+ Add</button>
      </div>

      {err && <p style={{ color: 'var(--c-red)', fontSize: '.74rem', marginBottom: 8 }}>{err}</p>}
      ...
      {form && (
        <div className="rise" style={{ border: '1px solid var(--line)', borderRadius: 12, padding: 14, marginBottom: 12, display: 'grid', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-secondary btn-sm" style={{ padding: '8px 18px' }} onClick={() => setForm(null)}>Cancel</button>
            <button className="btn btn-gold" style={{ padding: '8px 18px' }} disabled={busy || !form.name || !Number(form.amount)} onClick={save}>
              {busy ? 'Saving…' : 'Save'}</button>
          </div>
        </div>
      )}

      {rows.length === 0 && !form && <p style={{ fontSize: '.78rem', color: 'var(--c-muted)' }}>Debt-free — or not added yet. Liabilities reduce net worth and shape the Projector.</p>}
      {rows.map((l) => (
        <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid var(--line)', gap: 8 }}>
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: '.64rem', color: 'var(--c-muted)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 600, color: 'var(--c-red)' }}>−{f(l.amount)}</span>
            <button className="btn btn-xs btn-secondary" style={{ padding: '4px 10px' }} onClick={() => setForm({ ...l })}>✎</button>
            <button className="btn btn-xs btn-secondary" style={{ padding: '4px 10px' }} onClick={() => remove(l)}>✕</button>
          </div>
        </div>
      ))}
    </div>
