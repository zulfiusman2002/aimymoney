  ...
  return (
    <div style={{ marginTop: 34 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
        <div>
          <div className="t-label">Other wealth · property, land, gold, pension, cash</div>
          <h2 style={{ fontSize: '1.5rem', marginTop: 4 }}>Beyond the brokers</h2>
        </div>
        <button className="btn btn-primary" onClick={() => { setEditingId(null); setForm(blank(base)); }}>+ Add asset</button>
      </div>
      ...
      {form && (
        <div className="card fade-up" style={{ marginBottom: 18 }}>
          <div className="t-label">{editingId ? 'Edit asset' : 'New asset'}</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16, flexWrap: 'wrap', gap: 10 }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.3rem', fontWeight: 600 }}>
              = {f(converted(form.original_value, form.original_currency === base ? 1 : form.fx_rate))} <span className="t-label">in {base}</span>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="btn btn-secondary" onClick={() => { setForm(null); setEditingId(null); }}>Cancel</button>
              <button className="btn btn-gold" disabled={busy || !form.name || !Number(form.original_value)} onClick={submitForm}>
      ...
      {assets === null ? <div className="skeleton" style={{ height: 160 }} /> :
        assets.length === 0 && !form ? (
          <div className="card empty">
            ...
            <button className="btn btn-gold" style={{ marginTop: 18 }} onClick={() => setForm(blank(base))}>Add your first asset</button>
          </div>
        ) : (
              ...
              return (
                <div key={a.id} className="card" style={{ display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                    <span className="t-label">{classLabel(a.asset_class)}</span>
                    <span className="badge badge-neutral">{LIQUIDITY_LABELS[a.liquidity]}</span>
                  </div>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem', marginTop: 6 }}>{a.name}</div>
                  <div className="num-xl" style={{ fontSize: '1.7rem', marginTop: 2 }}>{f(a.converted_value)}</div>
                  ...
                  {delta != null && (
                    <div style={{ fontSize: '.74rem', marginTop: 6, color: delta >= 0 ? 'var(--c-green)' : 'var(--c-red)' }}>
                  ...
                  {isUpd && (
                    <div className="fade-up" style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--c-border)', display: 'grid', gap: 10 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button className="btn btn-secondary" style={{ padding: '8px 18px' }} onClick={() => setUpdating(null)}>Cancel</button>
                        <button className="btn btn-gold" style={{ padding: '8px 18px' }} disabled={busy || !Number(updating.value)} onClick={submitValuation}>
                  ...
                  {openHistory === a.id && hist.length > 0 && (
                    <div className="fade-up" style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--c-border)' }}>
                      {hist.slice(0, 6).map((v, i) => {
                        ...
                        return (
                          <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.72rem', padding: '4px 0' }}>
                            <span style={{ color: 'var(--c-muted)' }}>{v.valuation_date} · {v.source}</span>
                            <span>{f(v.converted_value)}{d != null && (
                              <span style={{ color: d >= 0 ? 'var(--c-green)' : 'var(--c-red)', marginLeft: 6 }}>
                                {d >= 0 ? '+' : ''}{f(d)}</span>)}</span>
                          </div>
                        );
                      })}
