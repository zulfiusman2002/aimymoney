  ...
  return (
    <div className="page">
      <div className="fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16, marginBottom: 26 }}>
        <div>
          <div className="t-label">Duolingo for your money · Psychology of Money × Atomic Habits</div>
          <h1 style={{ fontSize: '2.3rem', marginTop: 6 }}>Learn</h1>
        </div>
        <div style={{ display: 'flex', gap: 26, textAlign: 'right' }}>
          <div><div className="num-xl" style={{ fontSize: '1.8rem' }}>{streak?.current_streak ?? 0}<span className="unit"> 🔥</span></div>
            <div className="t-label">day streak</div></div>
          <div><div className="num-xl" style={{ fontSize: '1.8rem' }}>{streak?.total_xp ?? 0}</div>
            <div className="t-label">XP</div></div>
          <div><div className="num-xl" style={{ fontSize: '1.8rem' }}>{progress.length}</div>
            <div className="t-label">lessons done</div></div>
        </div>
      </div>

      {stage === 'loading' && <div className="skeleton" style={{ height: 320 }} />}
      {stage === 'error' && <div className="card" style={{ color: 'var(--c-red)' }}>{err} <button className="chip" onClick={load} style={{ marginLeft: 10 }}>Retry</button></div>}
      ...
      {(stage === 'lesson' || stage === 'quiz' || stage === 'done') && lesson && (
        <div className="card fade-up" style={{ maxWidth: 760, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            <span className="badge badge-good">Today's lesson · Module {lesson.module_id}</span>
            {completedToday && stage === 'lesson' && <span className="badge badge-neutral">streak already safe today</span>}
          </div>
          <h2 style={{ fontSize: '1.9rem', marginTop: 14 }}>{lesson.title}</h2>
          <p style={{ marginTop: 6, fontSize: '.78rem', color: 'var(--c-gold)', fontStyle: 'italic' }}>{card.reason}</p>

          {stage === 'lesson' && (
            <>
              <p style={{ marginTop: 18, fontSize: '.92rem', lineHeight: 1.75 }}>{lesson.content}</p>
              {lesson.example && (
                <div style={{ marginTop: 16, padding: 16, background: 'var(--c-gold-light)', borderRadius: 12, fontSize: '.82rem' }}>
                  <span className="t-label">Real life</span>
                  <p style={{ marginTop: 6 }}>{lesson.example}</p>
                </div>
              )}
              {lesson.reflection && (
                <p style={{ marginTop: 16, fontSize: '.84rem', fontStyle: 'italic', color: 'var(--c-muted)' }}>
                  Reflect: {lesson.reflection}
                </p>
              )}
              <button className="btn btn-gold" style={{ marginTop: 24 }} onClick={() => setStage('quiz')}>Take the quiz →</button>
            </>
          )}
          ...
          {stage === 'quiz' && (
            <>
              ...
              <button className="btn btn-gold" style={{ marginTop: 20 }} disabled={choice == null} onClick={submitQuiz}>Check answer</button>
            </>
          )}

          {stage === 'done' && result && (
            <div style={{ marginTop: 20 }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '2rem', color: result.correct ? 'var(--c-green)' : 'var(--c-amber)' }}>
              ...
              {lesson.action_challenge && (
                <div style={{ marginTop: 16, padding: 16, border: '1px dashed var(--c-gold)', borderRadius: 12, fontSize: '.84rem' }}>
                  <span className="t-label">Today's action</span>
                  <p style={{ marginTop: 6 }}>{lesson.action_challenge}</p>
                </div>
              )}
              <button className="btn btn-primary" style={{ marginTop: 22 }} onClick={load}>Next lesson →</button>
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ marginTop: 30 }}>
        <div className="t-label" style={{ marginBottom: 12 }}>Your path</div>
            ...
            return (
              <div key={m.id} className="card" style={{ padding: 18, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: '.72rem', color: 'var(--c-muted)' }}>{m.description}</div>
                </div>
