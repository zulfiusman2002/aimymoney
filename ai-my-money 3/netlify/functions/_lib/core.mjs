...
// Shared backend utilities + lightweight per-analysis context builders.
// ANTHROPIC_API_KEY and the service role key exist ONLY here — never in browser code.

import { createClient } from '@supabase/supabase-js';

const MODEL = process.env.ANTHROPIC_MODEL || 'claude-sonnet-4-6';
const CLAUDE_TIMEOUT_MS = 18_000;  // abort Anthropic call after 18 s
...
// ── Anthropic call with hard 18-second AbortController timeout ───────────────
export async function callClaude({ system, messages, maxTokens = 800 }) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), CLAUDE_TIMEOUT_MS);
  console.log('CALLING ANTHROPIC', { maxTokens, msgChars: messages.map((m) => String(m.content).length) });
  try {
    ...
    console.log('ANTHROPIC RETURNED', { stopReason: data.stop_reason, outputTokens: data.usage?.output_tokens });
    return data.content.filter((b) => b.type === 'text').map((b) => b.text).join('\n');
  } catch (e) {
    if (e.name === 'AbortError') {
      throw httpError(504, 'AI analysis timed out. Try a smaller review.');
    }
    throw e;
  } finally {
    clearTimeout(timer);
...
// ── Helper: month-aware income rows (records win over standard) ───────────────
...
// Each fetches only what its analysis actually needs.
...
async function ctxBudget(db, userId) {
  ...
  const [profile, income, expenses, allocs, goals] = await Promise.all([
    ...
    db.from('goals').select('goal_name, target_amount, current_amount, monthly_contribution').eq('user_id', userId).eq('status', 'active'),
  ]);
  ...
  const totalAlloc = (allocs.data || []).reduce((a, x) => a + Number(x.amount), 0);
  const catLines = Object.entries(byCat).sort((a, b) => b[1] - a[1]).map(([k, v]) => `${k} ${f(v)}`).join(', ');
  const goalLines = (goals.data || []).map((g) => `${g.goal_name}: needs ${f(g.monthly_contribution)}/mo`).join('; ') || 'none';
  return `BUDGET CONTEXT — ${p.name || 'user'} — ${month} (${cur})
Income: ${f(totalIncome)} | Expenses: ${f(totalExp)} (fixed ${f(byType.fixed)}, variable ${f(byType.variable)}, one-time ${f(byType['one-time'])})
Net savings: ${f(net)} | Savings rate: ${rate}% | Allocated to savings: ${f(totalAlloc)}
By category: ${catLines || 'none recorded'}
Goal contributions committed: ${goalLines}`;
}

async function ctxGoals(db, userId) {
  const month = monthStr();
  const [profile, income, expenses, goals, assets] = await Promise.all([
    db.from('user_profiles').select('name, currency').eq('user_id', userId).maybeSingle(),
    fetchIncome(db, userId, month),
    db.from('expenses').select('amount').eq('user_id', userId).eq('month', month),
    db.from('goals').select('*').eq('user_id', userId).eq('status', 'active'),
    db.from('assets').select('name, converted_value, liquidity').eq('user_id', userId).eq('is_active', true),
  ]);
  const p = profile.data || {};
  const cur = p.currency || 'GBP';
  const sym = { GBP: '£', USD: '$', EUR: '€', INR: '₹' }[cur] || cur + ' ';
  ...
  const MS = 2629800000;
  const goalLines = (goals.data || []).map((g) => {
    ...
    const mNeeded = mc > 0 ? Math.ceil(rem / mc) : null;
    const mAvail = g.target_date ? Math.max(0, Math.round((new Date(g.target_date) - Date.now()) / MS)) : null;
    const track = mNeeded != null && mAvail != null ? (mNeeded <= mAvail ? 'ON TRACK' : 'BEHIND') : 'no deadline';
    return `- ${g.goal_name}: ${f(current)}/${f(target)} · ${f(mc)}/mo · ${track}${mNeeded != null ? ` · ${mNeeded} mo needed` : ''}${mAvail != null ? ` · ${mAvail} mo left` : ''}`;
  }).join('\n') || 'No active goals.';
  const assetLines = (assets.data || []).map((a) => `${a.name} (${a.liquidity}) ${f(a.converted_value)}`).join(', ') || 'none';
  return `GOALS CONTEXT — ${p.name || 'user'} — ${month} (${cur})
Net savings this month: ${f(net)} | Committed to goals: ${f(committed)} | Headroom: ${f(net - committed)}
Goals:\n${goalLines}
Liquid/semi-liquid assets available: ${assetLines}`;
}

async function ctxNetworth(db, userId) {
  const [profile, snaps, assets, liabs, prevSnap] = await Promise.all([
    db.from('user_profiles').select('name, currency').eq('user_id', userId).maybeSingle(),
    db.from('investment_snapshots').select('asset_type, total_value, converted_total, snapshot_date')
    ...
    db.from('liabilities').select('name, amount, interest_rate, type').eq('user_id', userId),
    db.from('monthly_snapshots').select('net_worth, total_invested, total_assets')
  ...
  const nw = invested + aRows.reduce((a, x) => a + Number(x.converted_value), 0) - totalLiab;
  const prev = prevSnap.data;
  return `NET WORTH CONTEXT — ${p.name || 'user'} (${cur})
Net worth: ${f(nw)} | Investments: ${f(invested)} | Other assets: ${f(aRows.reduce((a, x) => a + Number(x.converted_value), 0))} | Liabilities: -${f(totalLiab)}
Liquidity: liquid ${f(liquid)} · semi ${f(semi)} · illiquid ${f(illiquid)}
Investments by type: ${Object.entries(latest).map(([t, s]) => `${t} ${f(s.converted_total ?? s.total_value)}`).join(', ') || 'none'}
Other assets: ${aRows.map((a) => `${a.name} (${a.asset_class}) ${f(a.converted_value)}`).join(', ') || 'none'}
Liabilities: ${(liabs.data || []).map((l) => `${l.name} ${f(l.amount)}${l.interest_rate ? ` @ ${l.interest_rate}%` : ''}`).join(', ') || 'none'}
${prev ? `Last month net worth: ${f(prev.net_worth)} (change: ${f(nw - Number(prev.net_worth))})` : 'No prior monthly snapshot.'}`;
}

async function ctxPortfolio(db, userId) {
  const [profile, snaps] = await Promise.all([
    db.from('user_profiles').select('name, currency').eq('user_id', userId).maybeSingle(),
    db.from('investment_snapshots').select('asset_type, total_value, converted_total, snapshot_date, currency, extraction_confidence, investment_holdings(asset_name,ticker,current_value,converted_value,gain_loss,confidence_score,platform)')
      .eq('user_id', userId).order('snapshot_date', { ascending: false }).limit(20),
  ]);
  const p = profile.data || {};
  const cur = p.currency || 'GBP';
  ...
  const lines = Object.values(latest).map((s) => {
    ...
    const holdings = (s.investment_holdings || []).slice(0, 5)
      .map((h) => `${h.asset_name}${h.ticker ? ` (${h.ticker})` : ''} ${f(h.converted_value ?? h.current_value)}${h.gain_loss != null ? ` G/L ${f(h.gain_loss)}` : ''}`).join(', ');
    return `- ${s.asset_type}: ${f(v)} [${age}d old${age > 45 ? ' ⚠ STALE' : ''}]${holdings ? ` — ${holdings}` : ''}`;
  });
  return `PORTFOLIO CONTEXT — ${p.name || 'user'} (${cur})
...
async function ctxSavingsOrRisk(db, userId) {
  const month = monthStr();
  const [profile, income, expenses, allocs, assets, liabs, snaps] = await Promise.all([
    db.from('user_profiles').select('name, currency').eq('user_id', userId).maybeSingle(),
    fetchIncome(db, userId, month),
  ...
  for (const s of snaps.data || []) if (!latest[s.asset_type]) latest[s.asset_type] = s;
  const invested = Object.values(latest).reduce((a, s) => a + Number(s.converted_total ?? s.total_value), 0);
  const totalAssets = (assets.data || []).reduce((a, x) => a + Number(x.converted_value), 0);
  const totalLiab = (liabs.data || []).reduce((a, l) => a + Number(l.amount), 0);
  const liquid = (assets.data || []).filter((a) => a.liquidity === 'liquid').reduce((a, x) => a + Number(x.converted_value), 0);
  return `SAVINGS/RISK CONTEXT — ${p.name || 'user'} — ${month} (${cur})
Income: ${f(totalIncome)} | Expenses: ${f(totalExp)} | Net savings: ${f(net)} | Savings rate: ${totalIncome > 0 ? ((net / totalIncome) * 100).toFixed(1) : 'n/a'}%
Savings allocations: ${(allocs.data || []).map((a) => `${a.destination} ${f(a.amount)}`).join(', ') || 'none'}
Assets: ${(assets.data || []).map((a) => `${a.name} (${a.asset_class}) ${f(a.converted_value)}`).join(', ') || 'none'} | Liquid: ${f(liquid)}
Investments total: ${f(invested)} | Liabilities total: ${f(totalLiab)} | Net worth approx: ${f(invested + totalAssets - totalLiab)}
Portfolio mix: ${Object.entries(latest).map(([t, s]) => `${t} ${f(s.converted_total ?? s.total_value)}`).join(', ') || 'none'}`;
}

// ── Full-review: uses buildContext (all data, no holdings subquery explosion) ─
export async function buildContext(db, userId) {
  const month = monthStr();
  const [profile, incomeRec, incomeStd, expenses, savingsAlloc, goals, liabilities, snaps, assets, prevSnap] =
    await Promise.all([
      ...
      // NO nested investment_holdings here — just snapshot totals
      db.from('investment_snapshots').select('asset_type, converted_total, total_value, snapshot_date')
        .eq('user_id', userId).order('snapshot_date', { ascending: false }).limit(20),
      db.from('assets').select('name, asset_class, liquidity, converted_value, original_currency, fx_date').eq('user_id', userId).eq('is_active', true),
      db.from('monthly_snapshots').select('total_income, total_expenses, savings_rate, net_worth')
        .eq('user_id', userId).eq('month', prevMonthStr()).maybeSingle(),
    ]);
  const p = profile.data || {};
  const cur = p.currency || 'GBP';
  ...
  const invested = Object.values(latest).reduce((a, s) => a + Number(s.converted_total ?? s.total_value), 0);
  const totalAssets = (assets.data || []).reduce((a, x) => a + Number(x.converted_value), 0);
  const totalLiab = (liabilities.data || []).reduce((a, l) => a + Number(l.amount), 0);
  ...
  const goalLines = (goals.data || []).map((g) => {
    ...
    return `${g.goal_name}: ${f(g.current_amount)}/${f(g.target_amount)}${mn ? ` (${mn} mo at current pace)` : ''}`;
  }).join('; ') || 'none';
  return `FULL REVIEW — ${p.name || 'user'} — ${month} (${cur})
Income: ${f(totalIncome)} | Expenses: ${f(totalExp)} | Net: ${f(net)} | Savings rate: ${rate}%
Top expenses: ${exps.sort((a, b) => b.amount - a.amount).slice(0, 5).map((e) => `${e.description} ${f(e.amount)}`).join(', ') || 'none'}
Savings allocated: ${(savingsAlloc.data || []).map((a) => `${a.destination} ${f(a.amount)}`).join(', ') || 'none'}
Investments: ${Object.entries(latest).map(([t, s]) => `${t} ${f(s.converted_total ?? s.total_value)}`).join(', ') || 'none'} | Total: ${f(invested)}
Other assets: ${(assets.data || []).map((a) => `${a.name} ${f(a.converted_value)}`).join(', ') || 'none'}
Liabilities: ${(liabilities.data || []).map((l) => `${l.name} ${f(l.amount)}`).join(', ') || 'none'} | Total: -${f(totalLiab)}
Net worth: ${f(nw)}${prev ? ` (prev month: ${f(prev.net_worth)}, Δ ${f(nw - Number(prev.net_worth))})` : ''}
Goals: ${goalLines}`;
}

// ── INTELLIGENCE COMPUTATION (for intelligence.mjs and learning-card.mjs) ────
export async function computeIntelligence(db, userId) {
  ...
  const [profile, incomeRec, incomeStd, expenses, savingsAlloc, goals, liabilities, snaps, assets, progress, streak, prevSnap, modules] =
    await Promise.all([
      ...
      db.from('investment_snapshots').select('asset_type, converted_total, total_value, snapshot_date, investment_holdings(asset_name,current_value,converted_value)')
        .eq('user_id', userId).order('snapshot_date', { ascending: false }).limit(30),
  ...
  for (const x of portfolio) {
    if (LIQUID.has(x.type)) liquid += x.value; else if (SEMI.has(x.type)) semi += x.value;
  }
  ...
  const goalStates = (goals.data || []).map((g) => {
    ...
    return { id: g.id, name: g.goal_name, target, current, pct, monthly: mc, targetDate: g.target_date,
      monthsNeeded, monthsAvailable, onTrack, requiredMonthly, eta };
  });
...
// ── Single export used by analyze.mjs ────────────────────────────────────────
export async function buildAnalysisContext(db, userId, type) {
  if (type === 'budget' || type === 'savings') return ctxBudget(db, userId);
  if (type === 'goals') return ctxGoals(db, userId);
  if (type === 'networth' || type === 'risk') return ctxNetworth(db, userId);
  if (type === 'portfolio' || type === 'changes') return ctxPortfolio(db, userId);
  return buildContext(db, userId);   // full-review uses the compact full context
}
