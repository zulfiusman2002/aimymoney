...
export default function WealthComposition({ comp, sym, base }) {
  ...
  const segs = [
    ['Liquid', comp.liquid, 'var(--brass)'],
    ['Semi-liquid', comp.semi, 'var(--green)'],
  ...
  return (
    <div className="card rise" style={{ marginBottom: 22 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 10 }}>
        <span className="eyebrow">Total wealth composition · {base}</span>
        <div>
          <span className="stat-value" style={{ fontSize: '2rem' }}>{f(comp.netWorth)}</span>
          <span className="eyebrow"> net worth</span>
        </div>
      </div>

      {/* stacked liquidity bar */}
      <div style={{ display: 'flex', height: 14, borderRadius: 8, overflow: 'hidden', marginTop: 16, background: 'var(--line)' }}>
      ...
      <div style={{ display: 'flex', gap: 18, marginTop: 10, flexWrap: 'wrap' }}>
        {segs.map(([name, v, color]) => (
          <span key={name} style={{ fontSize: '.72rem', color: 'var(--muted)' }}>
