  ...
  return (
    <div className="page">
      <div className="rise" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16, marginBottom: 26 }}>
        <div>
          <div className="eyebrow">Screenshot in · structured wealth out</div>
          <h1 style={{ fontSize: '2.3rem', marginTop: 6 }}>Investments</h1>
        </div>
        <div style={{ display: 'flex', gap: 22, alignItems: 'flex-end' }}>
          <div style={{ textAlign: 'right' }}>
            <div className="stat-value" style={{ fontSize: '1.9rem' }}>{f(total)}</div>
            <div className="eyebrow">tracked total ({base})</div>
          </div>
          <button className="btn brass" onClick={() => { reset(); setStep(1); }}>Update investments</button>
        </div>
      </div>
      ...
      {err && <div className="card" style={{ borderColor: 'var(--risk)', color: 'var(--risk)', marginBottom: 16, fontSize: '.8rem' }}>{err}</div>}

      {/* ---------- wizard ---------- */}
      {step > 0 && (
        <div className="card rise" style={{ marginBottom: 24 }}>
          {step === 1 && (<>
            <div className="eyebrow">Step 1 of 3 · What are you updating?</div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button className="btn brass" onClick={() => setStep(2)}>Continue</button>
            </div>
          </>)}

          {step === 2 && (<>
            <div className="eyebrow">Step 2 of 3 · Upload a screenshot of your {label(assetType)}</div>
            <label htmlFor="shot" style={{
              display: 'block', marginTop: 14, padding: preview ? 0 : '48px 20px',
              border: '2px dashed var(--line)', borderRadius: 14, textAlign: 'center',
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button className="btn brass" disabled={!file || busy} onClick={runExtraction}>Read screenshot →</button>
            </div>
          </>)}
          ...
          {step === 4 && extract && (<>
            <div className="eyebrow">Step 3 of 3 · Review &amp; approve</div>
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginTop: 12, alignItems: 'flex-end' }}>
              {extract.platform && <span className="badge good">Platform: {extract.platform}</span>}
              <span className="badge">Currency: {extract.currency}</span>
            ...
            {extract.warnings?.length > 0 && (
              <p style={{ marginTop: 10, fontSize: '.74rem', color: 'var(--warn)' }}>⚠ {extract.warnings.join(' · ')}</p>
            )}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '.78rem' }}>
                <thead><tr>
                  {['Holding', 'Ticker', 'Qty', `Value (${extract.currency})`, 'Invested', 'Gain/Loss', 'Conf.', ''].map((h) => (
                    <th key={h} style={{ textAlign: 'left', padding: '8px 10px', borderBottom: '1px solid var(--line)', fontSize: '.62rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--muted)' }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                    ...
                    <tr key={i}>
                      {[['asset_name', 'text', 160], ['ticker', 'text', 80], ['quantity', 'number', 70],
                        ...
                        <td key={k} style={{ padding: '6px 6px' }}>
                          <input type={type} value={h[k] ?? ''} style={{ width: w, fontFamily: 'var(--font-mono)', fontSize: '.76rem', padding: '8px 10px', border: '1px solid var(--line)', borderRadius: 8, background: 'var(--card)' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18, flexWrap: 'wrap', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button className="btn brass" disabled={busy} onClick={approve}>{busy ? 'Saving…' : 'Approve & save snapshot'}</button>
              </div>
            </div>
          </>)}
          ...
          {step === 5 && (<>
            ...
            {comparison ? (
              <p style={{ marginTop: 10, fontSize: '.86rem' }}>
                ...
                <span style={{ color: comparison.delta >= 0 ? 'var(--good)' : 'var(--risk)' }}>
            ...
            <p style={{ fontSize: '.72rem', color: 'var(--muted)', marginTop: 6 }}>Monthly snapshot and cross-module insights are refreshing in the background.</p>
            <button className="btn" style={{ marginTop: 16 }} onClick={reset}>Done</button>
          </>)}
        </div>
      )}
              ...
              return (
                <div key={s.asset_type} className="card">
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span className="eyebrow">{label(s.asset_type)}</span>
                    <span className={`badge ${ageDays > 45 ? 'warning' : ''}`}>{ageDays === 0 ? 'today' : `${ageDays}d ago`}</span>
                  </div>
                  <div className="stat-value" style={{ fontSize: '1.7rem', marginTop: 8 }}>{f(val)}</div>
                  {prevVal != null && (
                    <div style={{ fontSize: '.74rem', color: val >= prevVal ? 'var(--good)' : 'var(--risk)' }}>
                      {val >= prevVal ? '▲' : '▼'} {f(Math.abs(val - prevVal))} since {prev.snapshot_date}
                    </div>
                  )}
                  {(s.investment_holdings || []).slice(0, 3).map((h) => (
                    <div key={h.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.74rem', marginTop: 8, paddingTop: 8, borderTop: '1px solid var(--line)' }}>
                      <span>{h.asset_name}</span>
                      <span style={{ color: 'var(--muted)' }}>{f(h.converted_value ?? h.current_value)}</span>
                    </div>
                  ))}
                  {(s.investment_holdings || []).length > 3 && (
                    <div style={{ fontSize: '.68rem', color: 'var(--muted)', marginTop: 6 }}>+{s.investment_holdings.length - 3} more · {history.length} snapshot{history.length === 1 ? '' : 's'}</div>
                  )}
                </div>
              );
            })}
