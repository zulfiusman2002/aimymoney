...
const FlowNode = ({ x, y, width, height, payload }) => (
  <Layer>
    <Rectangle x={x} y={y} width={width} height={height} fill={payload.color || 'var(--green)'} radius={3} />
    <text x={x < 200 ? x + width + 8 : x - 8} y={y + height / 2} textAnchor={x < 200 ? 'start' : 'end'}
      dominantBaseline="middle" style={{ fontFamily: 'JetBrains Mono', fontSize: 11, fill: '#2b2a26' }}>
      {payload.name}
    </text>
  </Layer>
);
...
export default function Budget() {
  ...
  const sankey = useMemo(() => {
    ...
    const nodes = [{ name: `Income ${f(totals.ti)}`, color: 'var(--green)' }];
    const links = [];
    ...
    if (savings > 0) {
      nodes.push({ name: `Savings ${f(savings)}`, color: 'var(--brass)' });
  ...
  const Editor = () => {
    ...
    return (
      <div className="card rise" style={{ marginBottom: 18 }}>
        <div className="eyebrow">{row.id ? 'Edit' : 'Add'} {table === 'alloc' ? 'savings allocation' : table}</div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
          ...
          <button className="btn brass" disabled={busy} onClick={save}>{busy ? 'Saving…' : 'Save'}</button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="page">
      ...
      <div className="rise" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 14, marginBottom: 24 }}>
        <div>
          <div className="eyebrow">Where the month went — and where it should go</div>
      ...
      {incomeSource === 'standard' && income.length > 0 && (
        <div className="card rise" style={{ marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, borderColor: 'var(--brass)' }}>
          ...
          <button className="btn" style={{ padding: '8px 20px', fontSize: '.72rem' }} disabled={busy}
            onClick={async () => {
              setBusy(true);
      ...
      {expenses.length === 0 && prevExpenses.some((e) => e.recurring || e.type === 'fixed') && (
        <div className="card rise" style={{ marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, borderColor: 'var(--brass)' }}>
          ...
          <button className="btn" style={{ padding: '8px 20px', fontSize: '.72rem' }} onClick={copyPrevious}>Import fixed expenses</button>
        </div>
      )}

      {totals.ta > Math.max(0, totals.net) && totals.ti > 0 && (
        <div className="card" style={{ marginBottom: 14, borderColor: 'var(--warn)', fontSize: '.78rem', color: 'var(--warn)' }}>
      ...
      <div className="grid g4">
        {[['Income', f(totals.ti)], ['Expenses', f(totals.te)], ['Net savings', f(totals.net)]].map(([l, v]) => (
          <div key={l} className="card"><div className="eyebrow">{l}</div>
            <div className="stat-value" style={{ marginTop: 6 }}>{v}</div>
            {l === 'Expenses' && totals.prevTotal > 0 && (
              <div style={{ fontSize: '.72rem', color: totals.te <= totals.prevTotal ? 'var(--good)' : 'var(--risk)' }}>
        ...
        <div className="card" style={{ position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: 26 }}>
            <span className="stat-value" style={{ fontSize: '1.7rem' }}>{totals.rate.toFixed(0)}%</span>
          </div>
        </div>
      </div>
      ...
      {sankey && (
        <div className="card" style={{ marginTop: 18 }}>
          <div className="eyebrow" style={{ marginBottom: 8 }}>Money flow · {monthName(month)}</div>
      ...
      <div className="grid g2" style={{ marginTop: 18 }}>
        <div className="card">
          ...
          {catData.length ? (
            <ResponsiveContainer width="100%" height={Math.max(180, catData.length * 38)}>
              <BarChart data={catData} layout="vertical" margin={{ left: 6 }}>
                <XAxis type="number" hide /><YAxis type="category" dataKey="name" width={90} tick={{ fontSize: 11, fontFamily: 'JetBrains Mono' }} axisLine={false} tickLine={false} />
          ...
          ) : <div className="empty"><div className="display">Nothing yet</div><p>Add this month's expenses below.</p></div>}
        </div>
        <div className="card">
          <div className="eyebrow" style={{ marginBottom: 12 }}>Top movers vs last month</div>
          {totals.movers.length ? totals.movers.map((m) => (
            <div key={m.cat} style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0', borderBottom: '1px solid var(--line)', fontSize: '.82rem' }}>
              <span style={{ textTransform: 'capitalize' }}>{m.cat}</span>
              <span style={{ color: m.delta > 0 ? 'var(--risk)' : 'var(--good)' }}>
                {m.delta > 0 ? '+' : ''}{f(m.delta)} <span style={{ color: 'var(--muted)' }}>({f(m.prev)} → {f(m.now)})</span>
              </span>
            </div>
          )) : <div className="empty"><p>No data for {monthName(shiftMonth(month, -1))} to compare against.</p></div>}
        </div>
      </div>
      ...
      <div className="grid g3" style={{ marginTop: 18 }}>
        {[
          ...
          <div key={table} className="card">
            ...
            {rows.length === 0 && <p style={{ fontSize: '.76rem', color: 'var(--muted)' }}>Nothing recorded{table !== 'income' ? ` for ${monthName(month)}` : ''}.</p>}
            {rows.map((r) => {
              const [a, b, c] = fmt2(r);
              return (
                <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--line)', gap: 8 }}>
                  <div style={{ minWidth: 0 }}>
                    ...
                    {c && <div style={{ fontSize: '.64rem', color: 'var(--muted)' }}>{c}</div>}
                  </div>
      ...
      <div className="card" style={{ marginTop: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div className="eyebrow">AI budget insights</div>
          ...
          <button className="btn" onClick={runInsights} disabled={busy}>{busy ? 'Analysing…' : 'Analyse my budget'}</button>
        </div>
          ...
          <div className="grid g3" style={{ marginTop: 16 }}>
            {insights.insights.map((ins, i) => (
              <div key={i} style={{ border: '1px solid var(--line)', borderRadius: 12, padding: 14 }}>
                ...
                <div style={{ fontSize: '.74rem', color: 'var(--muted)' }}>{ins.detail}</div>
              </div>
            ))}
          </div>
        )}
        {insights?.disclaimer && <p style={{ marginTop: 12, fontSize: '.62rem', color: 'var(--muted)' }}>{insights.disclaimer}</p>}
      </div>
    </div>
  );
}
