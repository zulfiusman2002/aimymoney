  ...
  return (
    <div className="page">
      <div className="rise" style={{ marginBottom: 24 }}>
        <div className="eyebrow">Your private analyst</div>
      ...
      {err && <div className="card" style={{ borderColor: 'var(--risk)', color: 'var(--risk)', marginBottom: 18, fontSize: '.8rem' }}>{err}</div>}
      ...
      {analysis && !busyType && (
        <div className="card rise" style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
            ...
            {analysis.health_score != null && (
              <div style={{ textAlign: 'right' }}>
                <span className="stat-value" style={{ fontSize: '2rem' }}>{analysis.health_score}</span>
          ...
          {analysis.actions?.length > 0 && (
            <>
              <div className="eyebrow" style={{ margin: '22px 0 10px' }}>Recommended actions</div>
              {analysis.actions.map((a, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, padding: '10px 0', borderTop: '1px solid var(--line)' }}>
                  ...
                  <div><strong style={{ fontSize: '.82rem' }}>{a.title}</strong>
                    <div style={{ fontSize: '.76rem', color: 'var(--muted)' }}>{a.detail}</div></div>
                </div>
              ))}
            </>
          )}
          ...
          {analysis.data_gaps?.length > 0 && (
            <p style={{ marginTop: 16, fontSize: '.74rem', color: 'var(--warn)' }}>
              Missing data: {analysis.data_gaps.join(' · ')}
            </p>
          )}
          <p style={{ marginTop: 14, fontSize: '.64rem', color: 'var(--muted)' }}>{analysis.disclaimer}</p>
        </div>
      )}

      <div className="card">
        <div className="eyebrow" style={{ marginBottom: 16 }}>Ask anything about your finances</div>
        <div style={{ minHeight: 120, maxHeight: 420, overflowY: 'auto', paddingRight: 4 }}>
          ...
          {chatBusy && <div className="chat-msg ai" style={{ color: 'var(--muted)' }}>Reading your numbers…</div>}
          <div ref={endRef} />
        </div>
        <form onSubmit={send} style={{ display: 'flex', gap: 10, marginTop: 14 }}>
          <input value={input} onChange={(e) => setInput(e.target.value)}
            placeholder="e.g. How can I increase my savings rate?"
            style={{ flex: 1, fontFamily: 'var(--font-mono)', fontSize: '.85rem', padding: '13px 16px', border: '1px solid var(--line)', borderRadius: 999, background: 'var(--card)' }} />
          <button className="btn" disabled={chatBusy || !input.trim()}>Send</button>
        </form>
        <p style={{ marginTop: 10, fontSize: '.62rem', color: 'var(--muted)' }}>
