...
const TreeCell = ({ x, y, width, height, name, value, sym, index }) => {
  ...
  return (
    <g>
      ...
      {width > 80 && height > 36 && (
        <>
          <text x={x + 10} y={y + 20} style={{ fontFamily: 'JetBrains Mono', fontSize: 10, fill: '#f6f2ea', letterSpacing: '.06em' }}>{name}</text>
          <text x={x + 10} y={y + 38} style={{ fontFamily: 'Cormorant Garamond', fontSize: 17, fontWeight: 600, fill: '#fffdf8' }}>{sym}{Number(value).toLocaleString('en-GB', { maximumFractionDigits: 0 })}</text>
        </>
      )}
    </g>
  );
  ...
  return (
    <div className="page">
      <div className="rise" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 14, marginBottom: 24 }}>
        <div>
          <div className="eyebrow">Everything you own, minus everything you owe</div>
          <h1 style={{ fontSize: '2.3rem', marginTop: 6 }}>Net Worth</h1>
        </div>
        {mom != null && (
          <div style={{ textAlign: 'right' }}>
            <div className="eyebrow">vs last month</div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 600, color: mom >= 0 ? 'var(--good)' : 'var(--risk)' }}>
              {mom >= 0 ? '+' : ''}{f(mom)}
            </div>
          </div>
        )}
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        <div className="eyebrow" style={{ marginBottom: 10 }}>Net worth trend · from monthly snapshots</div>
        {data.trend.length >= 2 ? (
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={data.trend} margin={{ left: 8, right: 8 }}>
              ...
              <YAxis tickFormatter={(v) => `${sym}${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 10, fontFamily: 'JetBrains Mono' }} axisLine={false} tickLine={false} width={52} />
          ...
          <div className="empty"><div className="display">The line starts next month</div>
            <p style={{ maxWidth: 420, margin: '8px auto 0' }}>Monthly snapshots build the trend automatically as you use the app. {data.trend.length === 1 ? 'One point recorded so far.' : 'No points recorded yet — run a Full Review on the Dashboard to capture this month.'}</p></div>
        )}
      </div>
      ...
      <div className="grid g2" style={{ marginBottom: 18 }}>
        <div className="card">
          <div className="eyebrow" style={{ marginBottom: 10 }}>Wealth allocation</div>
          {allocation.length ? (
            <ResponsiveContainer width="100%" height={280}>
              <Treemap data={allocation} dataKey="value" nameKey="name" isAnimationActive
                content={<TreeCell sym={sym} />} />
            </ResponsiveContainer>
          ) : <div className="empty"><p>Add investments or assets to see allocation.</p></div>}
        </div>
        <div style={{ display: 'grid', gap: 18, alignContent: 'start' }}>
          <Liabilities rows={data.liabilities} onChanged={load} />
          {liabByType.length > 1 && (
            <div className="card">
              <div className="eyebrow" style={{ marginBottom: 10 }}>Liability breakdown</div>
              <ResponsiveContainer width="100%" height={liabByType.length * 42}>
                <BarChart data={liabByType} layout="vertical" margin={{ left: 6 }}>
                  <XAxis type="number" hide /><YAxis type="category" dataKey="name" width={110} tick={{ fontSize: 11, fontFamily: 'JetBrains Mono' }} axisLine={false} tickLine={false} />
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div className="eyebrow">AI net worth review</div>
          ...
          <button className="btn" onClick={runAI} disabled={busy}>{busy ? 'Reviewing…' : 'Review my net worth'}</button>
        </div>
        ...
        {ai?.disclaimer && <p style={{ marginTop: 12, fontSize: '.62rem', color: 'var(--muted)' }}>{ai.disclaimer}</p>}
      </div>
    </div>
  );
}
